...
| 2026-09-06 | 9B | Schematic wiring preview + Check topology, keyboard/touch access, validity-level status, local engine-failure wording. Preview rejects mixed-domain and duplicate connection-sets; Check serializes `connections`; wiring is one undoable `connect` and reports “checking” then “connected”. Nodes/ports are focusable with larger hit targets, Esc cancels, From/To selectors and nudge buttons avoid pointer-only wiring/layout. Status pills distinguish syntax / under-over-spec / unconverted units / convergence / engine failure. Free quantities highlight schematic instances. | Done |
