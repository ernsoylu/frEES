...
- [ ] Measure baseline and post-change pilot tasks from review section 3/R15, recording participant experience, unaided completion, time, errors, and recovery. Targets: 4/5 users solve the scalar within 5 minutes, build the chain within 10, and recover a missing boundary within 3; no silent wrong variant, lost map/override, or pointer-only required action. Record unmet targets as follow-up work; do not claim user validation from automated tests. **Follow-up:** no pilot has been run; automated tests do not stand in for it.
...
| 2026-09-06 | 9C | Public member paths on GUESS/Variable Information (`HX.in.P` → `hx$in$p`); Check `instances`/`definitions` with source lines; nested children navigate to the parent scope; local COMPONENT completion and signatures; shadowing and T/t advisories. | Done |
| 2026-09-06 | 9D | Wizard related-model table for Fan/Compressor/Pump families; search tags without renaming types; named-parameter/`connect`/quotes as the default style; Save project vs Export equation text; three journeys plus mixer/closed-loop. **Pilot (R15) is follow-up — not claimed from tests.** | Done |
