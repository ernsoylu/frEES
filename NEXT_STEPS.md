...
| 2026-09-06 | 9A | Corrected generated source and component assistance (R1–R5). `generateComponentText` appends units only to plain numeric literals; variable/annotated/expression values are preserved. Catalog, completion names, and reference pages are generated from this port's parsed library (`web/scripts/parse-library.mjs`) — 312 engine = catalog = pages in a standalone checkout; `npm run check-docs` enforces the name-set. Scaffolded the 17 missing moist-air pages. Signature help resolves `Type Instance(` (multiline, named args, strings/comments); highlighter treats `"…"` as comments and recognizes COMPONENT/PARAM/VARIANT/REQUIRE/CONNECT/LINEARIZE/GUESS. Wizard uses the engine `model$` default, unique case-insensitive instance names, field-level syntax errors, fluid suggestions, and quotes `INCOMP::MEG[0.50]`. Inactive variant inputs stay as visible drafts and emit a Check/Solve advisory without rewriting source. Guide claims about ~295 components and silent inactive-parameter rejection updated. Evidence: vitest generator/signature/WASM-boundary 56 pass; `check-docs` 655/655; expander + check advisory unit tests pass; clippy `-D warnings` clean; `tsc --noEmit` clean. 9B–9D remain. | Done |
